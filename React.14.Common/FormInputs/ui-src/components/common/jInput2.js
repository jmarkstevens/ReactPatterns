...
class JInputRender extends Component {
 	render() {
		...
		let returnRadio = (type={inputType} style={inputSty} checked={radioChecked} value={radioValue} onChange={this.handleValueChange})

		let returnChecked = 'type={inputType} style={inputSty} checked={checkedValue} onChange={this.handleCheckedChange}'

		let returnColor = 'type={inputType} style={inputSty} value={colorValue} onChange={this.handleValueChange}'

		let returnNumber = 'type={inputType} style={inputSty} value={numberValue} min={min} max={max} step={step} onChange={this.handleValueChange}'

		let returnText = 'type={inputType} style={inputSty} value={textValue} onChange={this.handleTextValueChange}'
		let returnIt = {};
		switch (inputType) {
		...
		return (<input ref='inputRef' {...returnIt} />);
	}
}
